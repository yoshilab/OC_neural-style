    ...
    with g.as_default(), g.device(device_t), \
        ...
        for i in range(num_iters):
            ...
            for j, path_out in enumerate(curr_batch_out):
                #output = save_img(path_out, _preds[j])
                output = _preds[j]
...
def ffwd_to_img(in_path, out_path, checkpoint_dir, device='/cpu:0'):
    ...
    output = ffwd(paths_in, paths_out, checkpoint_dir, batch_size=1, device_t=device)
    return output
...
def build_parser(checkpoint, in_path, out_path):
    parser = ArgumentParser()
    parser.add_argument('--checkpoint', type=str,
                        dest='checkpoint_dir',
                        help='dir or .ckpt file to load checkpoint from',
                        metavar='CHECKPOINT', default=checkpoint)

    parser.add_argument('--in-path', type=str,
                        dest='in_path',help='dir or file to transform',
                        metavar='IN_PATH', default=in_path)
...
def generate(checkpoint, in_path, out_path):
    parser = build_parser(checkpoint, in_path, out_path)
    ...
    if not os.path.isdir(opts.in_path):
        ...
        if opts.allow_different_dimensions:
            ...
            output = ffwd(full_in, full_out, opts.checkpoint_dir, device_t=opts.device,
                    batch_size=opts.batch_size)
            print('c\n')
    """
    output = ffwd_to_img(opts.in_path, out_path, opts.checkpoint_dir,
                    device=opts.device)
